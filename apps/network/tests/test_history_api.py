...
class RouteHistoryApiTestCase(TestCase):
    ...
    def test_history_returns_list(self):
        resp = self.client.get("/api/network/routes/history")
    ...
    def test_history_filters_by_source(self):
        resp = self.client.get("/api/network/routes/history?source=ServerA")
        self.assertEqual(resp.status_code, 200)
        body = resp.json()
        self.assertEqual({r["destination"] for r in body}, {"ServerB", "ServerC"})

    def test_history_filters_by_destination(self):
        resp = self.client.get("/api/network/routes/history?destination=ServerC")
        self.assertEqual(resp.status_code, 200)
        body = resp.json()
        self.assertEqual({r["source"] for r in body}, {"ServerA", "ServerB"})

    def test_history_limit(self):
        resp = self.client.get("/api/network/routes/history?limit=1")
        self.assertEqual(resp.status_code, 200)
        body = resp.json()
    ...
    def test_history_date_from_to(self):
        ...
        resp = self.client.get(
            f"/api/network/routes/history?date_from={date_from}&date_to={date_to}"
        )
        self.assertEqual(resp.status_code, 200)
        body = resp.json()
